//
//  FoodItemMockData.swift
//  MiniProjects
//
//  Created by Wannipa Reya on 15/5/2569 BE.
//

import Foundation

enum MockFoodData {

    static let heroItems: [FoodItem] = [
        FoodItem(title: "สลัดผัก", price: "80", imageName: "salad"),
        FoodItem(title: "ผัดไทย", price: "100", imageName: "pad_thai"),
        FoodItem(title: "ข้าวเหนียวมะม่วง", price: "120", imageName: "mango_sticky_rice")
    ]

    static let categoryItems: [FoodItem] = [
        FoodItem(title: "ก๋วยเตี๋ยว", price: "", imageName: "noodles"),
        FoodItem(title: "ข้าว", price: "", imageName: "rice"),
        FoodItem(title: "ของหวาน", price: "", imageName: "cake"),
        FoodItem(title: "เครื่องดื่ม", price: "", imageName: "bubble_tea"),
        FoodItem(title: "ของทอด", price: "", imageName: "fries")
    ]

    static let recommendedItems: [FoodItem] = [
        FoodItem(title: "พิซซ่า", price: "250", imageName: "pizza"),
        FoodItem(title: "เบอร์เกอร์", price: "180", imageName: "burger"),
        FoodItem(title: "ราเมน", price: "150", imageName: "ramen"),
        FoodItem(title: "ซูชิ", price: "390", imageName: "sushi")
    ]

    static let allItems: [FoodItem] = [
        FoodItem(title: "ผัดไทย", price: "100", imageName: "pad_thai"),
        FoodItem(title: "ก๋วยเตี๋ยว", price: "120", imageName: "noodles"),
        FoodItem(title: "ข้าวเปล่า", price: "20", imageName: "rice"),
        FoodItem(title: "ข้าวผัดกะเพรา", price: "100", imageName: "pad_kaprao"),
        FoodItem(title: "ราเมน", price: "150", imageName: "ramen"),
        FoodItem(title: "พิซซ่า", price: "250", imageName: "pizza"),
        FoodItem(title: "เบอร์เกอร์", price: "180", imageName: "burger"),
        FoodItem(title: "มันฝรั่งทอด", price: "90", imageName: "fries"),
        FoodItem(title: "เค้ก", price: "90", imageName: "cake"),
        FoodItem(title: "ข้าวเหนียวมะม่วง", price: "120", imageName: "mango_sticky_rice"),
        FoodItem(title: "ซูชิ", price: "390", imageName: "sushi"),
        FoodItem(title: "ติ่มซำ", price: "150", imageName: "dimsum"),
        FoodItem(title: "น้ำเปล่า", price: "10", imageName: "water"),
        FoodItem(title: "ชานมไข่มุก", price: "90", imageName: "bubble_tea")
    ]
}
